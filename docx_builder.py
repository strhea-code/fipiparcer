...
# Настройки печатного макета. Шрифт не уменьшаем: проблему больших карточек
# решаем не масштабированием текста, а удалением принудительной высоты и
# лишних пустых переносов.
BODY_FONT_SIZE_PT = 10.5
HEADER_FONT_SIZE_PT = 10.0
ANSWER_FONT_SIZE_PT = 11.0
ANSWER_SQUARES_WIDTH_CM = 5.8

# ВАЖНО: раньше --per-page задавал минимальную высоту строки таблицы
# (6/9 см). Из-за этого карточки с коротким текстом растягивались с пустотой,
# а большие карточки могли разрываться между колонками/страницами. Теперь
# карточки идут по фактическому содержимому; --per-page оставлен для
# совместимости команд, но не растягивает таблицы.
CARD_HEIGHT_BY_PER_PAGE: dict[int, float | None] = {0: None, 4: None, 6: None}
# Картинки ограничиваем по высоте, чтобы карточки не занимали пол-листа.
# Текст при этом остаётся обычного размера.
MULTI_IMAGE_MAX_HEIGHT_CM = 3.2
SINGLE_IMAGE_MAX_HEIGHT_CM = 4.8
...
def build_docx(
    ...
    per_page: оставлен для совместимости CLI. Высота карточки теперь авто,
    чтобы Word не растягивал короткие карточки пустым местом.
    """
    doc = Document()
